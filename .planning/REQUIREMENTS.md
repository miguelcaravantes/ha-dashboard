...
- [ ] **TOOL-01**: Remove Prettier dependencies, config files, and IDE integrations
- [ ] **TOOL-02**: Implement ESLint Stylistic (@stylistic/eslint-plugin) for all code formatting
- [ ] **TOOL-03**: Enforce strict `@typescript-eslint/consistent-type-assertions` with `assertionStyle: 'never'`
- [ ] **DATA-04**: Refactor all existing type assertions (`as` and `<Type>`) to safe alternatives (type guards, unknown casting, or logic changes)
