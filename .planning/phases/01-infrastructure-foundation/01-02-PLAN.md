...
<task type="auto">
  ...
  <task type="auto">
    <name>Task 4: Verify Dev Server</name>
    <files>package.json</files>
    <action>Attempt to start the dev server using `npm run dev` and verify it can initialize (using a timeout to prevent hanging).</action>
    <verify>timeout 10s npm run dev || [ $? -eq 124 ]</verify>
    <done>Dev server starts and is reachable or times out gracefully (124) after confirming initialization.</done>
  </task>
